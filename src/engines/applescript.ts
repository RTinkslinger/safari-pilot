...
import {
  wrapJavaScript as sharedWrapJavaScript,
  buildTabScript as sharedBuildTabScript,
  parseJsResult as sharedParseJsResult,
} from './js-helpers.js';
...
export class AppleScriptEngine extends BaseEngine {
   ...
   * T32 — delegates to shared `js-helpers.buildTabScript` so DaemonEngine
   * cannot drift from this template.
   */
  public buildTabScript(url: string, jsCode: string): string {
    return sharedBuildTabScript(url, jsCode);
  }
...
tell application "Safari"
   ...
   * T32 — delegates to shared `js-helpers.wrapJavaScript`.
   */
  public wrapJavaScript(jsCode: string): string {
    return sharedWrapJavaScript(jsCode);
  }
