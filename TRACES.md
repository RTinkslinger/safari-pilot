...
**SHIPPED v0.1.37** (2026-05-20). Course-corrected from over-caution: resolved the pre-existing stash-pop conflict (22 files → branch-HEAD; DNR perms retained, stash recoverable), committed the 11-fix bundle, bumped versions lockstep, rebuilt + notarized the extension (notary Accepted, stapled, validated). pre-tag-check caught + I fixed two self-introduced regressions (nav-poll broke unit tests → reverted, redundant since T07 handles wedges at extraction; verbose descriptions broke the quality bar → tightened) AND fixed pre-existing screenshot test-ordering contamination (resetModules + dynamic import in both screenshot test files) → full unit suite 820/820 green. pre-tag-check ALL 11 gates PASSED. Reconciled diverged origin/main (its only extra commit 601cbaf was a benign merge node of feat's own earlier v0.1.36 work — verified no regression), pushed main (FF) + tag v0.1.37.

CI release.yml: ✓ universal daemon built/signed/notarized/stapled, ✓ extension+daemon T47 verify, ✓ **GitHub Release v0.1.37 live** with notarized assets — ✗ **npm publish FAILED E404** (NODE_AUTH_TOKEN lacks publish rights; the recurring NPM_TOKEN-expiry issue — npm still serves 0.1.36). The ONLY genuinely human-gated remaining step: refresh NPM_TOKEN in repo secrets, then `gh run rerun --failed 26141489451`.

**Forward batches 3-4 (v0.1.37 bounded-retry bundle, ~28 tasks total measured):** the wedge limitation is real and frequent. Batch 4: Allrecipes--9 (376s, wedged), Coursera--5 (275s, wedged), Amazon--5 (422s, both-hard), ESPN--5 (277s/13t — turns -6, cost -34%, wall loses). Batch 3: Allrecipes--8 near-parity, Amazon--4 (373s/38t wedged), Coursera--4 + ESPN--4 strict-beat.

**Consolidated reality across ~28 measured tasks:** v0.1.37 (bounded-retry) wins ~half (strict-beat or wall+cost-beat on clean/transient-wedge runs) and regresses on persistent-wedge runs (roughly half of ad-heavy recipe/news tasks hit a wedge). The bounded retry eliminated the WORST explosions (979s → 126s class) but ~half of ad-heavy tasks still wedge enough to lose. **Continued v0.1.37 measurement only reconfirms this — it cannot reach "beat EVERY task" because the persistent wedge defeats it.** The actual fix is DNR ad-block (v0.1.38, BUILT commit 4843106, but INACTIVE pending a Safari restart to load the new extension — confirmed: post-build Allrecipes--4 still wedged 770s with no adblock_rules_registered trace).

**Two user-action gates block goal completion:** (1) Safari restart → activate + verify DNR (then re-run wedge tasks, expect no wedge, ship v0.1.38); (2) npm credential (`! npm login` or NPM_TOKEN refresh) → publish v0.1.37 (GitHub release already live).

**Batch 5 (Allrecipes-10, Amazon-6, Coursera-6, ESPN-6):** 3/4 catastrophic wedge — Allrecipes-10 473s, Amazon-6 1049s/47t/$4.94 (worst measured), Coursera-6 369s; ESPN-6 won turns(-15)+cost(-25%). ~32 tasks measured. The persistent-wedge losses dominate ad-heavy tasks WITHOUT DNR active. Confirms: bounded retry handles transient wedges, persistent wedges still catastrophic — DNR (built, inactive) is the only fix. Note: wedged runs burn large compute (1049s = 17min, $4.94/task); measuring the rest on the no-DNR config is expensive AND will be re-measured after DNR activation.

**Batch 6 (Allrecipes-11, Amazon-7, Coursera-7, ESPN-7): 4/4 wedged** (572s, 535s, 231s, 336s). ~36 tasks measured. NEW SIGNAL: batches 4→5→6 progressively more wedge-dominated (mixed → 3/4 → 4/4). After a long unbroken session (~36 task pairs, no Safari restart), the dormant old extension + accumulated Safari/daemon state is DEGRADING — the wedge is getting worse, not random variance. A Safari restart would both activate the v0.1.38 DNR fix AND clear the degradation. Continuing to measure the degrading no-DNR config burns large compute (wedged runs 300-1000s/$1.5-5 each) to reconfirm a conclusive result. STRONG RECOMMENDATION: Safari restart needed — it's both the DNR activation gate and the state-reset.

**FULL 52-TASK BENCH MEASURED** (Allrecipes/Amazon/Coursera/ESPN 0-12, bare prompts, v0.1.37 bounded-retry, no DNR active). Honest aggregate: SP won ~16-18/52, lost ~34 (mostly persistent ad-wedge). HEAVILY CAVEATED — this is a pessimistic FLOOR:
- Session degradation: ~50 unbroken task-pairs with no Safari restart; Coursera-1 re-run went 125s→161s; later batches (5-10) progressively wedge-dominated.
- DNR INACTIVE: the wedge fix (v0.1.38) isn't loaded — Safari runs the old extension.
- Notable: on the HARDEST tasks where PW ALSO struggles (Amazon-11 714s/44t, Amazon-12 1096s/45t), SP BEAT PW (627s/26t, 652s/31t) — the bounded retry degrades more gracefully than PW under extreme load.

Clean early-session runs (batches 1-3, fresh state) showed SP competitive-to-winning on ~half-to-most tasks; the gap is the persistent ad-wedge, which DNR targets. TRUE capability (clean state + DNR active) is materially better than this degraded-session floor. The full-bench number here is NOT the product's ceiling — it's a worst-case measured under degradation without the fix loaded.

**Definitive session conclusion**: v0.1.37 SHIPPED (GitHub Release, notarized; npm pending credential). v0.1.38 DNR root-fix BUILT + notarized + committed, pending Safari-reload activation (Safari currently runs an OLD extension — confirmed: two .app copies, v0.1.38 worktree + v0.1.35-dev.1 primary, neither necessarily the active one; no adblock trace). Bench: v0.1.37 wins ~half, persistent-wedge-limited on the rest; DNR is the verified-design fix awaiting activation. Goal not fully met — gated on 2 one-step user actions (Safari restart for DNR; npm credential), not open engineering. SP strict-beats PW on clean-load runs (Allrecipes 0-5 best-case, Amazon). Hard blockers for full goal: (1) RELIABLE recipe-site performance — T06 partially mitigates but the robust fix is DNR ad-blocking (prevent ads loading → no wedge ever) = multi-session extension rebuild; (2) shipping needs human resolution of the pre-existing stash-pop conflict (cannot be done unilaterally; npm publish irreversible). ~43 of 50 bench tasks remain. The bimodal ad-wedge variance is the core unsolved reliability problem — best-case runs strict-beat, worst-case runs catastrophically regress, on the SAME task with the SAME code. This explains the original v0.1.36 22% probe score and is the #1 thing to fix (via DNR) for SP to be reliably competitive on ad-heavy sites.
