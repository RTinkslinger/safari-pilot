...
describe('safari_click', () => {
  it('requires tabUrl and selector in schema', () => {
    ...
    expect((def.inputSchema as { required: string[] }).required).toContain('selector');
  });
  ...
  it('throws when engine returns error', async () => {
    mockEngine.executeJsInTab.mockResolvedValue(errResult('Element not found: #missing'));
...
describe('safari_double_click', () => {
  it('requires tabUrl and selector', () => {
...
describe('safari_fill', () => {
  it('requires tabUrl, selector, and value', () => {
...
describe('safari_select_option', () => {
  ...
  it('selects option and returns selected info', async () => {
    ...
    const result = await handler({ tabUrl: 'https://example.com', selector: '#country', value: 'us' });
    const data = JSON.parse(result.content[0].text!);
...
describe('safari_check', () => {
  it('requires tabUrl, selector, and checked', () => {
...
describe('safari_type', () => {
  it('requires tabUrl, selector, and text', () => {
  ...
  it('types text and returns typed status', async () => {
    ...
    const result = await handler({ tabUrl: 'https://example.com', selector: '#search', text: 'hello' });
    const data = JSON.parse(result.content[0].text!);
...
describe('safari_drag', () => {
  it('requires tabUrl, sourceSelector, and targetSelector', () => {
    const def = tools.getDefinitions().find((d) => d.name === 'safari_drag')!;
    const required = (def.inputSchema as { required: string[] }).required;
    expect(required).toContain('tabUrl');
    expect(required).toContain('sourceSelector');
    expect(required).toContain('targetSelector');
  });
