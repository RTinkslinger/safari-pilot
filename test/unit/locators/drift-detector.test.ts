...
describe('T7b drift detector — src/locator.ts ↔ extension/locator.js', () => {
  ...
  describe('sentinel wiring', () => {
    ...
    // v0.1.35 T10: query_all per-element envelope MUST include `interactability`
    // in BOTH paths. Extension returns the structured object built by
    // buildInteractability(el); AppleScript fallback emits null. Asymmetry is
    // deliberate (AppleScript can't reach elementFromPoint/computed-style from
    // `do JavaScript`), but the KEY must exist in both so consumers can
    // detect degraded mode without crashing on a missing field.
    it('query_all per-element envelope includes interactability in both paths', () => {
      // Extension path: builder helper + items.push must include the field.
      expect(EXT_LOCATOR, 'extension/locator.js missing buildInteractability helper')
        .toContain('function buildInteractability(el)');
      expect(EXT_LOCATOR, 'extension/locator.js resolveLocatorAll item must include interactability')
        .toMatch(/interactability:\s*buildInteractability\(el\)/);
      // AppleScript fallback: the IIFE generated by generateQueryAllJs must
      // emit `interactability: null` per element.
      expect(TS_LOCATOR, 'src/locator.ts generateQueryAllJs must emit interactability:null')
        .toMatch(/interactability:\s*null/);
    });
