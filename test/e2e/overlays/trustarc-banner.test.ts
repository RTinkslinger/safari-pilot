/**
 * Task 14 — per-pattern: trustarc-banner (cookie-consent).
 *
 * Pattern signals: selector #truste-consent-track + aria-label containing "cookie".
 * Dismiss: click #truste-consent-button + verify node-removed.
 */
import { describe, it, expect, beforeAll, afterAll } from 'vitest';
import { createServer, type Server } from 'node:http';
import { callTool, type McpTestClient } from '../../helpers/mcp-client.js';
import { getSharedClient } from '../../helpers/shared-client.js';
import { startTrustarcBannerNegativeFixture } from '../../fixtures/overlays-negative/trustarc-banner.negative.js';
...
function startTrustarcPositiveFixture(): { server: Server; url: () => string } {
  const html = `<!DOCTYPE html>
<html><body>
<main><h1>Article</h1></main>
<div id="truste-consent-track" role="dialog" aria-label="Cookie preferences"
     style="position:fixed;bottom:0;left:0;right:0;background:#fff;padding:1em;z-index:9999">
  <p>We use cookies on this site.</p>
  <button id="truste-consent-button">Agree</button>
</div>
<script>
  document.getElementById('truste-consent-button').addEventListener('click', function(){
    var n = document.getElementById('truste-consent-track');
...
describe('pattern: trustarc-banner — positive + negative pair', () => {
  ...
  it('POSITIVE: dismisses trustarc-banner', async () => {
    const fixture = startTrustarcPositiveFixture();
    try {
      const result = await dismiss(fixture.url(), 'trustarc-pos');
      expect(result.dismissed.some((d) => d.id === 'trustarc-banner')).toBe(true);
    } finally {
      await new Promise<void>((r) => fixture.server.close(() => r()));
    }
  }, 60_000);

  it('NEGATIVE: does NOT dismiss legitimate banner reusing #truste-consent-track id', async () => {
    const fixture = startTrustarcBannerNegativeFixture();
    try {
      const result = await dismiss(fixture.url(), 'trustarc-neg');
      expect(result.dismissed.every((d) => d.id !== 'trustarc-banner')).toBe(true);
    } finally {
      await new Promise<void>((r) => fixture.server.close(() => r()));
    }
  }, 60_000);
