...
- **SKIP_OWNERSHIP_TOOLS** — `safari_list_tabs`, `safari_new_tab`, `safari_health_check` bypass tab ownership checks. `safari_navigate_back`/`safari_navigate_forward` go through the deferred ownership path and use positional targeting (`executeJsInTabByPosition`) for the post-navigation page info query.
