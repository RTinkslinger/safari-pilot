...
| **T67** wake-without-reconcile recurrence (post-T60) | `extension/background.js` (lifecycle / reconcile path) | Discovered 2026-05-03 during 5A.1 smoke. Repeatedly observed: `lastAlarmFireTimestamp` advances on schedule (alarms wake the event page) but `lastReconcileTimestamp` does NOT advance — `isConnected` stays false for hours. T60's `supersedePollLoop()` fix was supposed to resolve this. Recovery requires manual extension toggle in Safari Settings → Extensions. Possible different failure mode than T60: maybe `/connect` POST fails or hangs without aborting cleanly, leaving the daemon-side `isConnected` flag false. **Triage steps:** (a) capture browser-side trace during a live wedge (storage.local debug dump tool — note_1a says "wired in commit 1b" so should be available), (b) check whether `forceUnload` could be wired to background.js as a recovery path (per SD-33d, no `forceReloadExtension()` exists), (c) consider exponential backoff with hard reload after N failed reconciles. Discovered during 5A.1 smoke but is NOT a 5A.1 regression — same lifecycle infra as T60. |
