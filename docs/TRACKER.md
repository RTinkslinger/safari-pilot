...
| **T65** phase3-3.1 form-submission flake | `test/e2e/phase3-interaction.test.ts` (3.1) | Discovered 2026-05-02 during 5A.3 regression sweep. `safari_click` on httpbin.org `/forms/post` submit button fails ~50% of runs with `TAB_NOT_FOUND` cache miss BEFORE the click reaches the extension. Confirmed pre-existing (reproduces with 5A.3 changes stashed). Tab opens correctly via `safari_new_tab`, fill (3.2) succeeds against the same tab, but click (3.1) flakes the cache lookup. Likely a race between `tabs.onUpdated` cache write and the click's findByUrl lookup, or httpbin redirect that drifts the tab URL. Investigate during Phase 5A · 5A.13 (Cluster 1–7 e2e sweep) — possibly fold into 5A.5 (locator chaining) work since both touch the lookup path. NOT blocking 5A.3. |
